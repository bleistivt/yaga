...
class YagaPlugin extends Gdn_Plugin
{
    ...
    public function settingsController_afterRenderAsset_handler($sender)
    {
        ...
        if (
            $eventArguments["AssetName"] == "Content" &&
            $sender->OriginalRequestMethod == "index"
        ) {
            ...
            echo wrap(
                wrap(
                    wrap("Badges" . wrap($badgeCount, "strong"), "div"),
                    "li",
                    ["class" => "BadgeCount"]
                ) .
                    wrap(
    ...
    public function base_getAppSettingsMenuItems_handler($sender)
    {
        ...
        if (Gdn::config("Yaga.Reactions.Enabled")) {
            ...
            $menu->addLink(
                $section,
                Gdn::translate("Yaga.Reactions", "Reactions"),
                "action/settings",
                "Yaga.Reactions.Manage"
            );
        }
        if (Gdn::config("Yaga.Badges.Enabled")) {
            $menu->addLink(
                $section,
                Gdn::translate("Yaga.Badges", "Badges"),
                "badge/settings",
                "Yaga.Badges.Manage"
            );
        }
        if (Gdn::config("Yaga.Ranks.Enabled")) {
            $menu->addLink(
                $section,
                Gdn::translate("Yaga.Ranks", "Ranks"),
                "rank/settings",
                "Yaga.Ranks.Manage"
            );
        }
    ...
    public function base_afterDiscussionFilters_handler($sender)
    {
        ...
        echo wrap(
            anchor(
                sprite("SpBestOf", "SpMod Sprite") .
                    " " .
                    Gdn::translate("Yaga.BestContent"),
                "/best"
            ),
            "li",
            [
                "class" =>
                    $sender->ControllerName == "bestcontroller"
                        ? "Best Active"
                        : "Best",
            ]
        );
    ...
    public function profileController_afterUserInfo_handler($sender)
    {
        ...
        echo wrap(Gdn::translate("Yaga.Reactions", "Reactions"), "h2", [
            "class" => "H",
        ]);
        ...
        foreach ($actions as $action) {
            ...
            $count = $reactionModel->getUserCount(
                $user->UserID,
                $action->ActionID
            );

            $tempString = wrap(
                wrap(Gdn_Format::bigNumber($count), "span", [
                    "title" => $count,
                ]),
                "span",
                ["class" => "Yaga_ReactionCount CountTotal"]
            );
            $tempString .= wrap($action->Name, "span", [
                "class" => "Yaga_ReactionName CountLabel",
            ]);
     ...
     * @param int $page
     */
    public function profileController_best_create(
        $sender,
        $userReference = "",
        $username = "",
        $page = 0
        ...
        list($offset, $limit) = offsetLimit(
            $page,
            Gdn::config("Yaga.BestContent.PerPage", 10)
        );
        if (!is_numeric($offset) || $offset < 0) {
            $offset = 0;
        }
    ...
    public function base_transformDiscussionToComment_handler($sender, $args)
    {
        Gdn::getContainer()
            ...
            ->update(
                [
                    "ParentID" => $args["TargetComment"]["CommentID"],
                    "ParentType" => "comment",
                ],
                [
                    "ParentID" => $args["SourceDiscussion"]["DiscussionID"],
    ...
    {
        if (Gdn::config("Yaga.Badges.Enabled")) {
            $sender->Preferences["Notifications"][
                "Email.BadgeAward"
            ] = Gdn::translate("Yaga.Badges.Notify");
            $sender->Preferences["Notifications"][
                "Popup.BadgeAward"
            ] = Gdn::translate("Yaga.Badges.Notify");
        }
        ...
        if (Gdn::config("Yaga.Ranks.Enabled")) {
            $sender->Preferences["Notifications"][
                "Email.RankPromotion"
            ] = Gdn::translate("Yaga.Ranks.Notify");
            $sender->Preferences["Notifications"][
                "Popup.RankPromotion"
            ] = Gdn::translate("Yaga.Ranks.Notify");
        }
    ...
    public function profileController_beforeProfileOptions_handler($sender)
    {
        if (Gdn::session()->isValid()) {
            if (
                Gdn::config("Yaga.Badges.Enabled") &&
                checkPermission("Yaga.Badges.Add")
            ) {
                $sender->EventArguments["ProfileOptions"][] = [
                    "Text" =>
                        sprite("SpBadge", "SpMod Sprite") .
                        " " .
                        Gdn::translate("Yaga.Badge.Award"),
            ...
            if (
                Gdn::config("Yaga.Ranks.Enabled") &&
                checkPermission("Yaga.Ranks.Add")
            ) {
                $sender->EventArguments["ProfileOptions"][] = [
                    "Text" =>
                        sprite("SpMod") .
                        " " .
                        Gdn::translate("Yaga.Rank.Promote"),
    ...
    {
        if (
            !Gdn::session()->checkPermission("Yaga.Reactions.View") ||
            !Gdn::config("Yaga.Reactions.Enabled")
        ) {
            return;
        }
        $type = "discussion";
    ...
    public function activityController_afterActivityBody_handler($sender)
    {
        ...
        if (
            !property_exists($activity, "AllowComments") ||
            $activity->AllowComments == 0
        ) {
            return;
        }
        ...
        if ($currentUserID == $activity->RegardingUserID) {
            ...
            echo wrap(renderReactionList($id, $type), "div", [
                "class" => "Reactions",
            ]);
        }
    }
    ...
    private function applyCustomConfigs($name = null, $value = null)
    {
        Gdn::config()->saveToConfig(
            "Yaga.ConfBackup." . $name,
            Gdn::config($name, null),
            ["Save" => false]
        );
    ...
    public function discussionController_render_before($sender)
    {
        ...
        if (Gdn::config("Yaga.Reactions.Enabled")) {
            ...
            if (isset($sender->Data["Comments"])) {
                $commentIDs = array_column(
                    $sender->Data["Comments"]->resultArray(),
                    "CommentID"
                );
    ...
    {
        Gdn::getContainer()
            ->get(BadgeAwardModel::class)
            ->executeHooks($sender, $handler);
    }
    ...
    public function base_render_before($sender)
    {
        if ($sender->MasterView == "admin") {
            ...
            if (
                Gdn::session()->isValid() &&
                is_object($sender->Menu) &&
                Gdn::config("Yaga.MenuLinks.Show")
            ) {
    ...
    {
        $sender->Data["Jobs"]["Recalculate User.CountBadges"] =
            "/yaga/dba/countbadges.json?";
        $sender->Data["Jobs"]["Recalculate Reaction.Latest"] =
            "/yaga/dba/latestreaction.json?";
        if (Gdn::config("Yaga.Dba.PointRecalculation")) {
            $sender->Data["Jobs"]["Recalculate User.Points"] =
                "/yaga/dba/userpoints.json?";
        }
    }
    ...
    public function setup()
    {
        if (Gdn::config("EnabledApplications.Yaga")) {
            throw new Gdn_UserException(
                'Please disable your old Yaga installation under "Applications" and remove the applications/yaga folder before enabling this plugin.'
            );
        }
