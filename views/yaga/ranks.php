...
foreach ($this->data("Ranks") as $rank) {
    ...
    if ($rank->Enabled) {
        ...
        if ($rank->PostReq > 0) {
            $reqs[] = sprintf(
                Gdn::translate("Yaga.Ranks.Story.PostReq"),
                $rank->PostReq
            );
            $posts = true;
        }
        if ($rank->PointReq > 0) {
            if ($posts) {
                $reqs[] = sprintf(
                    Gdn::translate("Yaga.Ranks.Story.PostAndPointReq"),
                    $rank->PointReq
                );
            } else {
                $reqs[] = sprintf(
                    Gdn::translate("Yaga.Ranks.Story.PointReq"),
                    $rank->PointReq
                );
            }
        }
        ...
        switch (count($reqs)) {
            case 3:
                $metaString = sprintf(
                    Gdn::translate("Yaga.Ranks.Story.3Reqs"),
                    $reqs[0],
                    $reqs[1],
                    $reqs[2]
                );
            ...
            case 2:
                $metaString = sprintf(
                    Gdn::translate("Yaga.Ranks.Story.2Reqs"),
                    $reqs[0],
                    $reqs[1]
                );
                break;
            case 1:
                $metaString = sprintf(
                    Gdn::translate("Yaga.Ranks.Story.1Reqs"),
                    $reqs[0]
                );
                break;
            default:
    ...
    //} else {
    $row .= img("plugins/yaga/design/images/default_promotion.png", [
        "class" => "RankPhoto",
    ]);
    //}

    $row .= wrap(
        wrap($rank->Name, "div", ["class" => "Title"]) .
            wrap($rank->Description, "div", ["class" => "Description"]) .
            wrap(
                wrapIf($metaString, "span", [
                    "class" => "MItem RankRequirements",
                ]),
                "div",
                ["class" => "Meta"]
            ),
