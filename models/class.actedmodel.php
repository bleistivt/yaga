...
class ActedModel extends Gdn_Model
{
    ...
    private function getItems(
        ...
        $this->EventArguments["PermissionColumns"] = [
            CategoryModel::defaultCategory()["PermissionCategoryID"],
        ];
        ...
        if ($method === "received" || $method === "taken") {
            $where =
                "where " .
                ($method === "received" ? "ParentAuthorID" : "InsertUserID") .
                " = " .
                intval($userID) .
                " and ActionID = " .
                intval($actionID);
        ...
        $order =
            "coalesce(" .
            ($method === "best" ? $scoreColumns : $dateColumns) .
            ")";
    ...
    protected function process($records)
    {
        ...
        foreach ($records as $record) {
            $item = $this->reactionModel->getReactionItem(
                $record["ParentType"],
                $record["ParentID"]
            );
            ...
            // Attach User
            $item["Author"] = Gdn::userModel()->getID(
