...
class YagaController extends DashboardController
{
    ...
    public function badgeDetail($badgeID, $slug = null)
    {
        ...
        $userBadgeAward = $this->BadgeAwardModel
            ->getWhere(["BadgeID" => $badgeID, "UserID" => $userID])
            ->firstRow();
        $recentAwards = $this->BadgeAwardModel->getRecent(
            $badgeID,
            Gdn::config("Yaga.Badges.RecentAwards")
        );
    ...
    public function import()
    {
        ...
        if (!class_exists("ZipArchive")) {
            $this->Form->addError(
                Gdn::translate("Yaga.Error.TransportRequirements")
            );
        }
    ...
    protected function _exportData($include = [], $path = null)
    {
        ...
        if ($fh->open($path, ZipArchive::CREATE) !== true) {
            $this->Form->addError(
                sprintf(
                    Gdn::translate("Yaga.Error.ArchiveCreate"),
                    $fh->getStatusString()
                )
            );
            return false;
        ...
        foreach ($filteredImages as $image) {
            ...
            if ($fh->addFile("./" . $image, "images/" . $image) === false) {
                $this->Form->addError(
                    sprintf(
                        Gdn::translate("Yaga.Error.AddFile"),
                        $fh->getStatusString()
                    )
                );
                //return false;
        ...
        if ($fh->close()) {
            ...
            $this->Form->addError(
                sprintf(
                    Gdn::translate("Yaga.Error.ArchiveSave"),
                    $fh->getStatusString()
                )
            );
            return false;
    ...
    protected function _importData($info, $include)
    {
        ...
        // Import Configs
        $configs = dbdecode(
            file_get_contents(PATH_UPLOADS . "/import/yaga/" . $info["Config"])
        );
        ...
        foreach ($include as $key => $value) {
            ...
            if ($value) {
                $data = dbdecode(
                    file_get_contents(
                        PATH_UPLOADS . "/import/yaga/" . $info[$key]
                    )
                );
        ...
        if (
            file_exists($path) &&
            Gdn_FileSystem::copy($path, PATH_UPLOADS . "/") === false
        ) {
    ...
    protected static function _nestedToDotNotation($configs, $prefix = "")
    {
        ...
        foreach ($configs as $name => $value) {
            if (is_array($value)) {
                $configStrings = array_merge(
                    $configStrings,
                    self::_nestedToDotNotation($value, "$prefix.$name")
                );
            } else {
    ...
    protected function _validateChecksum($metaData)
    {
        ...
        $hashes[] = md5_file(
            PATH_UPLOADS . "/import/yaga/" . $metaData["Config"]
        );

        // Hash the data files
        if (array_key_exists("Action", $metaData)) {
            $hashes[] = md5_file(
                PATH_UPLOADS . "/import/yaga/" . $metaData["Action"]
            );
        }

        if (array_key_exists("Badge", $metaData)) {
            $hashes[] = md5_file(
                PATH_UPLOADS . "/import/yaga/" . $metaData["Badge"]
            );
        }

        if (array_key_exists("Rank", $metaData)) {
            $hashes[] = md5_file(
                PATH_UPLOADS . "/import/yaga/" . $metaData["Rank"]
            );
        }
