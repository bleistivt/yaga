...
abstract class AbstractYagaBestApiController extends AbstractApiController
{
    ...
    protected function calculateOutput($data, $url, $query, $in)
    {
        ...
        $expandReactions = $this->resolveExpandFields($query, [
            "yagaReactions" => "ContentID",
        ]);
        if (!empty($expandReactions)) {
            $this->reactionModel->expandYagaReactions($data->Content, [
                "ContentID",
                "ItemType",
            ]);
        }
        ...
        $out = $this->schema(
            [
                ":a" => [
                    ...
                    "score:f" =>
                        'The current score of this item (see "yaga_setUserScore" event)',
