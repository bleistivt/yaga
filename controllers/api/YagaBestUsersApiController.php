...
class YagaBestUsersApiController extends AbstractYagaBestApiController {
    ...
    public function get_reactions(int $userID, int $actionID, array $query) {
        ...
        $in = $this->inputSchema('Get the content of a user which received a certain reaction.');
        $query = $in->validate($query);
        [$offset, $limit] = ApiUtils::offsetLimit($query);

        $data = $this->reactionModel->getBest(ReactionModel::ITEMS_PROFILE_REACTION, $limit, $offset, $actionID, $userID);

        return $this->calculateOutput($data, $userID.'/reactions/'.$actionID, $query, $in);
    }
    ...
    public function get_best(int $userID, array $query) {
        ...
        $data = $this->reactionModel->getBest(ReactionModel::ITEMS_PROFILE_BEST, $limit, $offset, false, $userID);
    ...
    public function get_totals(int $userID) {
        ...
        foreach ($actions as $action) {
            $action->Count = $this->reactionModel->getUserCount($userID, $action->ActionID);
            $data[] = ApiUtils::convertOutputKeys((array)$action);
        }

        $out = $this->schema([
            ':a' => YagaActionFragmentSchema::instance()->merge(Schema::parse([
                'permission:s' => 'The permission required to use this reaction',
                'count:i' => 'The number received of this reaction (user profiles)'
            ]))
        ], 'out');
