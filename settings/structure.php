...
$duplicateTableMessage =
    "Please check both the %s and %s tables. Delete the one which does NOT contain your data.";
...
if ($construct->tableExists()) {
    if ($construct->tableExists("YagaAction")) {
        throw new Gdn_UserException(
            sprintf($duplicateTableMessage, $px . "Action", $px . "YagaAction")
        );
    }
...
if ($construct->tableExists()) {
    if ($construct->tableExists("YagaBadge")) {
        throw new Gdn_UserException(
            sprintf($duplicateTableMessage, $px . "Badge", $px . "YagaBadge")
        );
    }
...
if ($construct->tableExists()) {
    if ($construct->tableExists("YagaRank")) {
        throw new Gdn_UserException(
            sprintf($duplicateTableMessage, $px . "Rank", $px . "YagaRank")
        );
    }
...
if ($construct->tableExists()) {
    $result = $sql
        ->query(
            "show index from {$px}YagaReaction where Key_name = 'UX_YagaReaction_Reaction'"
        )
        ->result();
    if (!$result) {
        $sql->query(
...
if ($construct->tableExists()) {
    $result = $sql
        ->query(
            "show index from {$px}YagaBadgeAward where Key_name = 'UX_YagaBadgeAward_Award'"
        )
        ->result();
    if (!$result) {
        $sql->query(
...
$construct
    ...
    ->column("ActionID", "int", false, [
        "index.Profile",
        "index.ProfileCount",
        "index.Best",
    ])
    ->column("ParentID", "int", false, ["index.Record", "unique.Reaction"])
    ->column("ParentType", "varchar(100)", false, [
        "index.Record",
        "unique.Reaction",
    ])
...
$construct
    ...
    ->column("Description", "varchar(255)", true) // deprecated
    ->column("Photo", "varchar(255)", true)
    ->column("Emoji", "varchar(255)", true)
    ->column("Tooltip", "varchar(255)")
    ->column("CssClass", "varchar(255)", true)
...
if (
    $sql->getWhere("ActivityType", ["Name" => "BadgeAward"])->numRows() == 0 &&
    !$construct->CaptureOnly
) {
    $sql->insert("ActivityType", [
        "AllowComments" => "1",
        "Name" => "BadgeAward",
        "FullHeadline" => '%1$s earned a badge.',
        "ProfileHeadline" => '%1$s earned a badge.',
        "Notify" => 1,
    ]);
}
if (
    $sql->getWhere("ActivityType", ["Name" => "RankPromotion"])->numRows() ==
        0 &&
    !$construct->CaptureOnly
) {
    $sql->insert("ActivityType", [
        "AllowComments" => "1",
        "Name" => "RankPromotion",
        "FullHeadline" => '%1$s was promoted.',
        "ProfileHeadline" => '%1$s was promoted.',
        "Notify" => 1,
    ]);
}
...
if (!$construct->CaptureOnly) {
    ...
    $sql->update(
        "YagaBadge",
        ["Photo" => $newPath . "default_badge.png"],
        ["Photo" => $oldPath . "default_badge.png"]
    )->put();
    $sql->update(
        "YagaBadge",
        ["Photo" => $newPath . "default_promotion.png"],
        ["Photo" => $oldPath . "default_promotion.png"]
    )->put();

    $sql->update("YagaBadge")
        ->set("Photo", "replace(`Photo`, '$oldPath', '$newPath')", false)
        ->put();

    $yagaTypes = array_column(
        $sql
            ->getWhere("ActivityType", [
                "Name" => ["BadgeAward", "RankPromotion"],
            ])
            ->resultArray(),
        "ActivityTypeID"
    );
    $sql->update("Activity")
        ->set("Photo", "replace(`Photo`, '$oldPath', '$newPath')", false)
        ->whereIn("ActivityTypeID", $yagaTypes)
        ->put();
}
